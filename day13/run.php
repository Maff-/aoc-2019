...
function runProgram(array $program): array
{
    ...
    $outputs = iterator_to_array($arcade, false);
    $outputs = array_chunk($outputs, 3);
    foreach ($outputs as [$x, $y, $tile]) {
        $tileMap[$y][$x] = $tile;
    }
...
$map = runProgram($program);

echo 'Result part1: ', array_count_values(array_merge(...$map))[2], PHP_EOL;
